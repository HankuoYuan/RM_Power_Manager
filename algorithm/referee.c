...
static unsigned char referee_process_status = 0,
                     referee_header_buffer[5] = {0xA5, [1 ... 4] = 0};
...
const unsigned int k_game_robot_status_size = 13;
...
void referee_process_buffer(const unsigned char *buffer) {
  ...
  for (int counter = 0; counter < REFEREE_DMA_BUFFER_SIZE; ++counter) {
    ...
    case 0:
      if (buffer[counter] == 0xA5)
        referee_process_status = 1;
      break;
    case 1:
      current_package.header.data_length = buffer[counter];
      referee_header_buffer[1] = buffer[counter];
      referee_process_status = 2;
      break;
    case 2:
      current_package.header.data_length |= (buffer[counter] << 8);
      ...
      break;
    case 3:
      current_package.header.package_sequence = buffer[counter];
    ...
    case 4:
      ...
      break;
    case 5:
      current_package.cmd_id = buffer[counter];
      referee_process_status = 6;
      break;
    case 6:
      current_package.cmd_id |= (buffer[counter] << 8);
      ...
      break;
    case 7:
      current_package.data[current_package.data_counter] = buffer[counter];
      current_package.data_counter++;
      if (current_package.data_counter == current_package.header.data_length)
        referee_process_status = 8;
      break;
    case 8:
      current_package.crc16_low = buffer[counter];
      referee_process_status = 9;
      break;
    case 9:
      crc16_expected = get_crc16_value(
          referee_header_buffer, current_package.cmd_id, current_package.data,
    ...
    case 10:
      current_package.data_counter++;
      if (current_package.data_counter ==
          current_package.header.data_length + 2)
        goto clear_all_referee_data;
      break;
    default:
...
static void referee_process_struct(referee_package_t *package) {
  ...
  case k_game_status_cmd:
    memcpy(&game_status, package->data, k_game_status_size);
    referee_info.game_progress = game_status.game_progress;
    referee_info.timestamp = game_status.time_stamp;
    break;
  case k_game_robot_status_cmd:
    memcpy(&game_robot_status, package->data, k_game_robot_status_size);
    ...
    break;
  case k_power_heat_data_cmd:
    memcpy(&power_heat_data, package->data, k_power_heat_data_size);
    referee_info.chassis_power = power_heat_data.chassis_power;
    referee_info.chassis_voltage =
        (float)power_heat_data.chassis_volt / 1000.0f;
    referee_info.chassis_current =
        (float)power_heat_data.chassis_current / 1000.0f;
